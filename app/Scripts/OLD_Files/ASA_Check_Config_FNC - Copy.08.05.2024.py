...
def Split_Show_run(Device, Show_Line, log_folder):
    hostname___ = Device.replace('/','___')
    log_folder = log_folder + '/' + hostname___

    # read previusly collected output
    try:
        with open("%s/%s.txt"%(log_folder,hostname___),"r") as f:
            l = f.readlines()
    except:
        print('file %s/%s.txt not found!' %(log_folder,hostname___))
    ...
    for n in range(0,len(l)):
        ...
        if line.startswith('!'):
            ...
            while not( (nn == len(l)-1) or (l[nn].startswith('show ')) ):
                ...
                nn += 1
    try:
        with open("%s/%s___%s.txt"%(log_folder,hostname___,Show_Line.title().strip().replace(' ','_')),"w") as f:
            for n in temp:
                f.write(n)
    except:
        print("Can't open file %s/%s___%s.txt for writing" %(log_folder,hostname___,Show_Line.title().strip().replace(' ','_')))
...
def Config_Diff(Device, Config_Change, log_folder):
    ...
    T_0_ShowRun_file   = log_folder + '/' + hostname___ + '.CFG.t-0.txt'
    T_1_ShowRun_file   = log_folder + '/' + hostname___ + '.CFG.t-1.txt'
    Delta_ShowRun_file = log_folder + '/' + hostname___ + '.CFG.Delta.txt'
...
##    confdiff_html      =  '../../app/templates/confdiff.html'
    #html_folder        = '../../app/templates/Log_FW/' + log_folder
    html_folder = log_folder
    ...
    Delta_File = []
    try:
        with open(T_1_ShowRun_file, mode="r") as f:
    ...
    if len(Diff_Only) > 0:
        ...
        try:
            tf_name = "%s/VAR_%s___%s"%(log_folder,hostname___,'Diff_Only_DF')
        ...
        try:
            with open("%s/%s"%(html_folder,Delta_ShowRun_html),mode="w") as html_file:
                ...
                html_file.write('''
                   ...
                   ''')
##                Red_Words    = ['NEW','show','run','|','i','ip','any','clear','tcp','udp','ip','icmp','eq','range','deny']
##                Blu_Words    = ['interface','access-group','access-list','host','network','nat']
##                Green_Words  = ['in','log','description','logging']
##                Purple_Words = ['configure', 'extended', 'permit']
##                Brown_Words  = ['network-object','source','dynamic','static','destination','object-group','object']
##                Red_Color    = '#ba1e28'
##                Blu_Color    = '#1e25ba'
##                Green_Color  = '#1cb836'
##                Purple_Color = '#8f1489'
##                Brown_Color  = '#995c00'
                ...
                for t_col_index in range(0,N_Cols):
                    html_file.write('           <th>%s</th>\n' %Diff_Only_DF.columns[t_col_index])
                ...
                for row in Diff_Only_DF.itertuples():
                    ...
                    for t_col_index in range(0,N_Cols):
                        ...
                        if t_col_index == N_Cols-1:
                            new_line = ''
                            for t_word in Diff_Only_DF.iloc[row.Index][t_col_index].split():
...
##    # write confdiff.html
##    try:
##        with open("%s"%(confdiff_MASTER),mode="r") as f:
##            l = f.readlines()
##    except:
##        raise OSError("Can't read file (%s)!" %(confdiff_MASTER))
##
##    try:
##        with open("%s"%(confdiff_html),mode="w") as f:
##            for line in l:
##                if ('<!-- TABLE_GOES_HERE -->') in line:
##                    for m in html_file:
##                        f.write(m)
##                else:
##                    f.write(l)
##    except:
##        raise OSError("Can't write file (%s)!" %(confdiff_html))
    ...
    ConfLenHist_FList = []
    ConfLenHist_FName = ('%s/%s-ConfLenHist.txt' %(log_folder,hostname___))
    ...
    ##  if ConfLenHist.txt exist:
            with open(ConfLenHist_FName,"r") as f:
...
##          left_shift the array for delta values
    ...
    try:
        with open("%s/chart-area1.js"%(Err_folder),"r") as f:
            l = f.readlines()
    except:
        print('ERROR!!! file %s/chart-area1.js not found!' %(Err_folder))
...
def Get_ASA_Commands(Device,log_folder):
    ...
    while retries <= 3:
        try:
            device_connection = ConnectHandler(**nexus)
    ...
    if retries >= 3:
        ...
        print("FAILED TO CONNECT TO %s@%s" %(Device[4],Device[0]))

    retries = 0
    output = []

    if nexus['device_type'] == 'cisco_ftd':
...
##        device_connection.send_command('enable')
    ...
    if hostname != Device[4]:
        print('ATTENZIONE!!!! Hostname mismatch!!')
        print('This Device is %s while connected device is %s' %(Device[4], hostname))
        exit(123)
    ...
    for t_Command in Commands:
        ...
        if retries == 3:
            output.append("UNABLE TO RUN COMMAND %s on %s" %(t_Command, hostname))
    ...
    try:
        with open("%s/%s.txt"%(log_folder,hostname___),"w+") as f:
            for n in output:
                f.write('\n\n!_________________________________________________________\n\n')
                f.write(n)
                f.write('\n\n')
        print('... saved file "%s/%s.txt" '%(log_folder,hostname___))
    except:
        raise OSError("Can't write to destination file (%s/%s.txt)!" % (log_folder,hostname___))
...
def ACL_VS_Interface(t_device, Config_Change, log_folder):
    ...
    global WTF_Error_FName
    Watch_FList = []
    Watch_FName = log_folder + '/' + hostname___ + '-Unprotected_IF-Watch.html'
    Think_FList = []
    Think_FName = log_folder + '/' + hostname___ + '-Unprotected_IF-Think.html'
    Fix_FList   = []
    Fix_FName   = log_folder + '/' + hostname___ + '-Unprotected_IF-Fix.html'
    Merge_Flist = []
    Merge_FName = log_folder + '/' + hostname___ + '-Unprotected_IF-Merge.txt'
    ...
    try:
        engine = db.create_engine("postgresql://postgres:igiull0_123@localhost/ASA_Check")
        connection = engine.connect()
        ACL_Summary = db.Table('ACL_Summary', db.MetaData(), autoload=True, autoload_with=engine)
        My_Devices = db.Table('My_Devices', db.MetaData(), autoload=True, autoload_with=engine)
    except:
        print('=================[ Warning ]==================')
        print('DB not connected, some feature is unavailable\n')
    ...
    try:
        ...
        print('ERROR!!! file %s/%s___Show_Nameif.txt not found!' %(log_folder,hostname___))
...
##            l = f.readlines()
##    except:
##        print('ERROR!!! file %s/%s___Show_Run_Access-Group.txt not found!' %(log_folder,hostname___))
##
...
##            Config_Change.append('Warning... No access-list configured on Interface "%s"' %n)
    ...
    if DB_Available:
        query = db.delete(ACL_Summary).where(ACL_Summary.columns.HostName=="%s" %hostname___)
        results = connection.execute(query)
    ...
    for t_item in Accessgroup_Dic_by_if.keys():
        ...
        if DB_Available:
            query = db.insert(ACL_Summary).values(  HostName = hostname___,
                                                    Nameif = t_item,
...
def NO_Log_For_ACL(t_device, Config_Change, log_folder):
    ...
    #Show_run_ACL_Inactive_Lst = []
    #Show_run_ACL_LogDis_Lst.append('\n!The following lines were having log disabled')

    log_folder = log_folder + '/' + hostname___
    nologacl_htm_FName = log_folder + '/' + hostname___ + '.nologacl_Fix.html'
    nologacl_txt_FList = []
    nologacl_txt_FName = log_folder + '/' + hostname___ + '.nologacl_Fix.txt'
    nologacl_txt_FList.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
    nologacl_txt_FList.append('!\n')
    logdisabledacl_htm_FName = log_folder + '/' + hostname___ + '.logdisabledacl_Fix.html'
    logdisabledacl_txt_FName = log_folder + '/' + hostname___ + '.logdisabledacl_Fix.txt'
    ...
    inactiveacl_htm_FName = log_folder + '/' + hostname___ + '.inactiveacl_Fix.html'
    inactiveacl_txt_FList = []
    inactiveacl_txt_FName = log_folder + '/' + hostname___ + '.inactiveacl_Fix.txt'
    ...
    try:
        ...
        print('ERROR!!! file %s/%s___Show_Running-Config.txt not found!' %(log_folder,hostname___))
        exit(0)
    tf_name = "%s/VAR_%s___%s"%(log_folder,hostname___,'Accessgroup_Dic_by_if')
    with shelve.open(tf_name) as shelve_obj: Accessgroup_Dic_by_if = shelve_obj['0']
    ...
    if logging_monitor_line != '':
        Config_Change.append('! logging monitor level configured is: "%s"' %logging_monitor_line)
    if logging_monitor_line != '':
...
##    retries = 0
##    if len(Show_run_ACL_NoLog_Lst) > 0:
##        while retries <3:
##            try:
##                with open(nologacl_htm_FName, "w+") as f:
##                    f.write('<code><nobr>\n')
##                    f.write('<br>\n'.join(Show_run_ACL_NoLog_Lst))
##                    f.write('</nobr></code>\n')
##                    print('saved file %s' %nologacl_htm_FName)
##                    break
##            except:
##                retries +=1
##                time.sleep(retries*2)
##        if retries == 3:
##            print('ERROR!!! Cannot write to file %s' %nologacl_htm_FName)
    with open(nologacl_txt_FName, "w") as f:
        f.write('\n'.join(nologacl_txt_FList))
        f.write('\n'.join(Show_run_ACL_NoLog_Lst))

##    retries = 0
##    if len(Show_run_ACL_Inactive_Lst) > 0:
##        while retries <3:
##            try:
##                with open(inactiveacl_htm_FName, "w+") as f:
##                    f.write('<code>\n')
##                    f.write('<br>\n'.join(Show_run_ACL_Inactive_Lst))
##                    f.write('</code>\n')
##                    print('saved file %s' %inactiveacl_htm_FName)
##                    break
##            except:
##                retries +=1
##                time.sleep(retries*2)
##        if retries == 3:
##            print('ERROR!!! Cannot write to file %s' %inactiveacl_htm_FName)
##    with open(inactiveacl_txt_FName, "w") as f:
##        f.write('\n'.join(inactiveacl_txt_FList))
##        f.write('\n'.join(Show_run_ACL_Inactive_Lst))
...
def Unused_ACL(t_device, Config_Change, log_folder):
    ...
    Watch_FName = log_folder + '/' + hostname___ + '-Unused_ACL-Watch.html'
    Think_FList = []
    Think_FName = log_folder + '/' + hostname___ + '-Unused_ACL-Think.html'
    Fix_FList   = []
    Fix_FName   = log_folder + '/' + hostname___ + '-Unused_ACL-Fix.html'
    Merge_Flist = []
    Merge_FName = log_folder + '/' + hostname___ + '-Unused_ACL-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
    Merge_Flist.append('!')
    ...
    with open("%s/%s___Show_Capture.txt"%(log_folder,hostname___),"r") as f:
        l = f.readlines()
    ACL_Capture_List = []
    for n in range(0,len(l)):
...
##    Config_Change.append('--- %s ACL over %s are not used (%s%%)\n'%(len(Unused_ACL_List), len(ACL_List), percent))
    ...
    if DB_Available:
        query = db.update(My_Devices).values(Unused_ACL=len(Unused_ACL_List))
        query = query.where(My_Devices.columns.HostName==hostname___)
        results = connection.execute(query)
        query = db.update(My_Devices).values(Declared_ACL=len(ACL_List))
        query = query.where(My_Devices.columns.HostName==hostname___)
        results = connection.execute(query)
        query = db.update(My_Devices).values(Percent_Unused_ACL=percent)
        query = query.where(My_Devices.columns.HostName==hostname___)
        results = connection.execute(query)
        connection.close()
        engine.dispose()
...
##            Config_Change.append('clear configure access-list %s' %n)
    ...
    tf_name = "%s/VAR_%s___%s"%(log_folder,hostname___,'Unused_ACL_List')
...
def Unused_Object(t_device, Config_Change, log_folder):
    ...
    try:
        with shelve.open(tf_name) as shelve_obj: Used_Object_List = shelve_obj['0']
    except:
        print('ERROR @ Unused_Object:..... File %s not found!' %tf_name)
        exit('Unused_Object exit ERROR.')
    ...
    try:
        with shelve.open(tf_name) as shelve_obj: Declared_OBJ_GRP_NET = shelve_obj['0']
    except:
        print('ERROR @ Unused_Object:..... File %s not found!' %tf_name)
        exit('Unused_Object exit ERROR.')
    ...
    try:
        with shelve.open(tf_name) as shelve_obj: Declared_Object_service = shelve_obj['0']
    except:
        print('ERROR @ Unused_Object:..... File %s not found!' %tf_name)
        exit('Unused_Object exit ERROR.')
...
##    Config_Change.append('!')
##    for n in Unused_Obj_Net:
##        Config_Change.append('no object network %s' %n)
##    Config_Change.append('!')
##    for n in Unused_ObjGrp_Net:
##        Config_Change.append('show run | i %s' %n)
##    Config_Change.append('!')
##    for n in Unused_ObjGrp_Net:
##        Config_Change.append('no object-group network %s' %n)


    # ----- WTF for Unused_Obj_Net ---------------------------------------------------
    Watch_FList = []
    Watch_FName = log_folder + '/' + hostname___ + '-ObjNet_Not_Applied-Watch.html'
    Think_FList = []
    Think_FName = log_folder + '/' + hostname___ + '-ObjNet_Not_Applied-Think.html'
    Fix_FList   = []
    Fix_FName   = log_folder + '/' + hostname___ + '-ObjNet_Not_Applied-Fix.html'
    Merge_Flist = []
    Merge_FName = log_folder + '/' + hostname___ + '-ObjNet_Not_Applied-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S\n\n'))

    Watch_FList.append('<p class="text-secondary" >')
    if len(Unused_Obj_Net) > 0:
        #Watch_FList.append('The following %s Network Object over %s are not applied:<br><ul>' %(Count_Obj_Not_Applied, len(Declared_OBJ_NET)))
        Watch_FList.append('<ul>')
        Merge_Flist.append('The following %s Network Object over %s are not applied:\n\n' %(Count_Obj_Not_Applied, len(Declared_OBJ_NET)))
    ...
    Write_Think_File(Think_FName, Think_FList)
    Write_Think_File(Fix_FName, Fix_FList)
    File_Save_Try(Merge_FName,Merge_Flist)

    # ----- WTF for Unused_ObjGrp_Net ---------------------------------------------------
    Watch_FList = []
    Watch_FName = log_folder + '/' + hostname___ + '-ObjGrpNet_Not_Applied-Watch.html'
    Think_FList = []
    Think_FName = log_folder + '/' + hostname___ + '-ObjGrpNet_Not_Applied-Think.html'
    Fix_FList   = []
    Fix_FName   = log_folder + '/' + hostname___ + '-ObjGrpNet_Not_Applied-Fix.html'
    Merge_Flist = []
    Merge_FName = log_folder + '/' + hostname___ + '-ObjGrpNet_Not_Applied-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S\n\n'))

    Watch_FList.append('<p class="text-secondary" >')
    if len(Unused_ObjGrp_Net) > 0:
        #Watch_FList.append('The following %s Network Object-Group over %s are not applied:<br><ul>' %(Count_ObjGrp_Not_Applied, len(Declared_OBJ_GRP_NET)))
        Merge_Flist.append('The following %s Network Object-Group over %s are not applied:\n\n' %(Count_ObjGrp_Not_Applied, len(Declared_OBJ_GRP_NET)))
        Watch_FList.append('<ul>')
    ...
    Watch_FList = []
    Watch_FName = log_folder + '/' + hostname___ + '-ObjSvc_Not_Applied-Watch.html'
    Think_FList = []
    Think_FName = log_folder + '/' + hostname___ + '-ObjSvc_Not_Applied-Think.html'
    Fix_FList   = []
    Fix_FName   = log_folder + '/' + hostname___ + '-ObjSvc_Not_Applied-Fix.html'
    Merge_Flist = []
    Merge_FName = log_folder + '/' + hostname___ + '-ObjSvc_Not_Applied-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S\n\n'))

    Watch_FList.append('<p class="text-secondary" >')
    if len(Unused_Obj_Service) > 0:
        #Watch_FList.append('The following %s Service Object over %s are not applied:<br><ul>' %(Count_ObjSrv_Not_Applied, len(Declared_Object_service)))
        Merge_Flist.append('The following %s Service Object over %s are not applied:\n\n' %(Count_ObjSrv_Not_Applied, len(Declared_Object_service)))
        Watch_FList.append('<ul>')
    ...
    Watch_FList = []
    Watch_FName = log_folder + '/' + hostname___ + '-ObjGrpSvc_Not_Applied-Watch.html'
    Think_FList = []
    Think_FName = log_folder + '/' + hostname___ + '-ObjGrpSvc_Not_Applied-Think.html'
    Fix_FList   = []
    Fix_FName   = log_folder + '/' + hostname___ + '-ObjGrpSvc_Not_Applied-Fix.html'
    Merge_Flist = []
    Merge_FName = log_folder + '/' + hostname___ + '-ObjGrpSvc_Not_Applied-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S\n\n'))

    Watch_FList.append('<p class="text-secondary" >')
    if len(Unused_ObjGrp_Service) > 0:
        #Watch_FList.append('The following %s Service/Protocol Object-Group over %s are not applied:<br><ul>' %(Count_ObjGrpSrv_Not_Applied, len(OBJ_GRP_SVC_Dic_2.keys())))
        Merge_Flist.append('The following %s Service/Protocol Object-Group over %s are not applied:\n\n' %(Count_ObjGrpSrv_Not_Applied, len(OBJ_GRP_SVC_Dic_2.keys())))
        Watch_FList.append('<ul>')
    ...
    #controlla oggetti duplicati
##    Duped_Objects_Dict = {}
##    for n in Obejct_by_value_Dict.keys():
##        if len(Obejct_by_value_Dict[n]) > 1:
##            Duped_Objects_Dict[n] = Obejct_by_value_Dict[n]
...
##    Watch_Heading_Text = ('%s Network Objects over %s (%s%%) are duplicated:' %(N_of_unique_Duplicated_OBJ_NET, len(Declared_OBJ_NET), Prcnt_N_of_unique_Dup_OBJ_NET))
    ...
    Watch_FName   = log_folder + '/' + hostname___ + '-ObjNet_Duplicated-Watch.html'
    try:
...
##            t_key = t_group[m]
##            Out = []
##            t_Out = Where_Used(t_device, t_key, log_folder, Out)
##            if t_Out:
...
##                    print(line)
    ...
    Watch_FName   = log_folder + '/' + hostname___ + '-ObjGrpNet_Duplicated-Watch.html'
    try:
        with open(Watch_FName,mode="w") as html_file:
            #html_file.writelines(line for line in Watch_Flist)
            html_file.writelines(Watch_Flist)
...
##            Config_Change.append('- object service "%s" declared in "%s"' %(t_key,temp))
    ...
    Watch_FName   = log_folder + '/' + hostname___ + '-ObjSvc_Duplicated-Watch.html'
    try:
        with open(Watch_FName,mode="w") as html_file:
            #html_file.writelines(line for line in Watch_Flist)
            html_file.writelines(Watch_Flist)
    ...
    for t_key in Duplicated_OBJ_SVC.keys():
        ...
        for t_item in Duplicated_OBJ_SVC[t_key]:
            ...
            Out = []
            t_Out = Where_Used(t_device, t_item, log_folder, Out)
            if t_Out:
                for line in t_Out:
...
##                        t_Config_Change.append(remove_tags.sub('', line.strip())+'\n')
    ...
    Think_FName   = log_folder + '/' + hostname___ + '-ObjSvc_Duplicated-Think.html'
    try:
...
##                    print(line)
    ...
    Watch_FName   = log_folder + '/' + hostname___ + '-ObjGrpSvc_Duplicated-Watch.html'
    try:
        with open(Watch_FName,mode="w") as html_file:
            #html_file.writelines(line for line in Watch_Flist)
            html_file.writelines(Watch_Flist)
...
##            print('Notify...  object %s declared as "network-object" but "object network" %s exists' %(n,Obejct_by_value_Dict[n]))
    ...
    Config_Change.append('Number of undeclared "network-object" = %s' %len(Undeclared_NetObj_List))
    Config_Change.append('Number of equivalent "object network" = %s' %len(Undeclared_NetObj_Used_List))

    tf_name = "%s/VAR_%s___%s"%(log_folder,hostname___,'Undeclared_NetObj_Used_List')
...
def ACL_Source_Vs_Routing_Table(t_device, Config_Change, log_folder):
    ...
    for row in ROUTE_IP_DF.itertuples():
        try:
            ROUTE_IP_DF.loc[row.Index].Network = ipaddress.IPv4Network(row.Network)
        except:
            ...
            for t_this_Src_Obj in this_Src_Obj:
                ...
                if WIDE_ROUTE_List != []:
                    ...
                    if text_line not in Printed_Lines:
                        ...
                        Config_Change.append(text_line)
                        Printed_Lines.append(text_line)
                    text_line = (' - Surce_Object is "%s", interface is "%s", routing is:' %(t_this_Src_Obj, t_If_Name))
                    if text_line not in Printed_Lines:
                        Config_Change.append(text_line)
                        Printed_Lines.append(text_line)
...
##                                        print('>>> Hash Duplicato @ %s' %(' '.join(row))) if (DEBUG_LEVEL == 0) else ''
    ...
    try:
        engine = db.create_engine("postgresql://postgres:igiull0_123@localhost/ASA_Check")
        connection = engine.connect()
        ACL_GROSS = db.Table('ACL_GROSS', db.MetaData(), autoload=True, autoload_with=engine)
        query = db.select([ACL_GROSS]).where(ACL_GROSS.columns.HostName=="%s" %hostname___)
        ResultProxy = connection.execute(query)
        ResultSet = ResultProxy.fetchall()
        ACL_GROSS_df = pd.DataFrame(ResultSet)
        ...
        ACL_GROSS_df = ACL_GROSS_df.drop('ID', 1)
    except:
        print('=================[ Warning ]==================')
        print('DB not connected, some feature is unavailable\n')
        Config_Change.append('=================[ Warning ]==================')
    ...
    for t_key in list(SiActive_Noroute_Hash_ACL_Dic.keys()):
        ...
        for row in ACL_Lines_DF.query(Bool_check).itertuples(): # un solo elemento
            ACL_Lines_DF.iloc[row.Index].Hitcnt = "(hitcnt=%s)" %row.Hitcnt
            ...
            t_ACL_Lines_DF_check = t_ACL_Lines_DF
...
##        print(n)
        Config_Change.append('! ' + n)
        command_line = 'no ' + n.split('log')[0]
        command_line =  command_line.split('(hitcnt')[0]
        Config_Change.append(command_line)
...
##        print('\n @ ' + t_key)
        ...
        if check_point not in Processed_ACL_line:
            ...
            #oggetto usato solo come souorce in questa ACL (una o più volte)
                ...
                Config_Change.append('Obj_Source "%s" in singola ACL' %t_OBJ_Src)
                if len(t_Where_OBJ_Dst_df) == 0:
                    Config_Change.append('Obj_Source in nessuna destination')
                    #oggetto mai usato come Dest
                    #print('Object can be removed...')

                    if  (t_OBJ_Src.split()[0]) == 'object':
                        this_src_OBJ = t_OBJ_Src.split()[1]
                        print(".. object... COME E' POSSIBILE???? Dovrebbe essere un totally wrong routing.... ")
                            ...
                            if 'host ' in src_to_find:
                                ...
                                for item in this_Obj_Grp:
                                    if 'network-object host' in item:
                                        ...
                                        # ----- host annidato, recursive lookup da gestire -----
                                        #Config_Change.append('----- host %s annidato, recursive lookup da gestire -----' %item)
                                        continue
                        ...
                        this_src_OBJ = t_OBJ_Src.split()[1]
                        print(".. host... COME E' POSSIBILE???? Dovrebbe essere un totally wrong routing.... ")
                    ...
                    Config_Change.append('Obj_Source "%s" used in Destination also...' %t_OBJ_Src)
                ...
                if check_line not in Processed_line:
                    ...
                    Config_Change.append('Object "%s" used as source in other ACL also @ %s ...' %(t_OBJ_Src, ', '. join(list(t_Where_OBJ_Src_df['Name'].unique()))))
                    Config_Change.append(tabulate(t_Where_OBJ_Src_df,t_Where_OBJ_Src_df,tablefmt='psql',showindex=False))
                    for row in Child_ACL_Df.itertuples():
                        src_to_find = row.Source
                        Config_Change.append('!no routing for %s' %src_to_find)
                    #piu' ACL con questo SRC_OBJ
                    if len(t_Where_OBJ_Dst_df) == 0:
                        Config_Change.append('Obj_Source %s in nessuna destination...' %t_OBJ_Src)
                        Config_Change.append(tabulate(t_Where_OBJ_Src_df,t_Where_OBJ_Src_df,tablefmt='psql',showindex=False))
...
### rifaer lo stesso per le inactive
...
def ACL_Dest_Vs_Routing_Table(t_device, Config_Change, log_folder):
    ...
    for row in ROUTE_IP_DF.itertuples():
        try:
            ROUTE_IP_DF.loc[row.Index].IPv4_Network = ipaddress.IPv4Network(row.Network)
        except:
            try:
                t_routename = row.Network.split('/')[0]
                t_routeip = Name_dic[t_routename] +'/'+ row.Network.split('/')[1]
                ROUTE_IP_DF.loc[row.Index].IPv4_Network = ipaddress.IPv4Network(t_routeip)
            except:
                Config_Change.append('ERROR 1105 while converting %s to ipaddress\n' %row.Network)
                print('ERROR 1105 while converting %s to ipaddress\n' %row.Network)
                ROUTE_IP_DF = ROUTE_IP_DF.drop(row.Index)
    ...
    BINS = 0; LOOP_INDEX = -1; STEPS = 10; ITEMS = ROUTE_IP_DF.shape[0]
    ...
    print('elapsed time is: %s' %str(end-start))

##    for row_index1, row1 in ROUTE_IP_DF.iterrows():
##        Routing_Space_IN[row1.Interface] += row1.IPv4_Network.num_addresses
##        for row_index2, row2 in ROUTE_IP_DF.iterrows():
...
##                    Routing_Space_IN[row2.Interface] -= row1.IPv4_Network.num_addresses
                    ...
                    #print('%s @ IF %s subnet of %s @ IF %s' %(row1.IPv4_Network, row1.Interface, row2.IPv4_Network, row2.Interface))
                #if row2.Network != '0.0.0.0/0':
                    #print('%s @ IF %s subnet of %s @ IF %s' %(row1.IPv4_Network, row1.Interface, row2.IPv4_Network, row2.Interface))
...
##        t_Child_Hash = []
        ...
        for row in t_ACL_Lines_DF.itertuples():
            ...
            if row.Action == 'permit':
                if utils_v2.ASA_ACL_Obj_to_IP(row.Source)[0] == ipaddress.IPv4Network('0.0.0.0/0'):
...
# from here
                BEST_ROUTE = ''
                for row_1 in ROUTE_IP_DF.itertuples():
                ...
                if t_this_Dst_Obj != '0.0.0.0/0':
                    if BEST_ROUTE != '': #no best route found
                        Out_Interface = ROUTE_IP_DF.loc[ROUTE_IP_DF['IPv4_Network'] == BEST_ROUTE].Interface.to_list()[0]
                        ACL_OUT_IF_COUNTER_dic[(t_If_Name,Out_Interface)] += 1
                        #ACL_OUT_IF_ACLs_dic[(t_If_Name,Out_Interface)].append(' '.join(row))
                        ACL_OUT_IF_ACLs_dic[(t_If_Name,Out_Interface)].append(ACL_text)
                        ...
                        ROUTE_IP_DF_bis = ROUTE_IP_DF.copy()
                        Best_Route_Index = ROUTE_IP_DF.index[ROUTE_IP_DF['IPv4_Network'] == BEST_ROUTE].to_list()[0]
                    ...
                    if row.Action == 'permit':
                        for t_OUT_ifName in Nameif_List:
                            ...
                            #ACL_OUT_IF_ACLs_dic[(t_If_Name,t_OUT_ifName)].append(' '.join(row))
                ...
                if WIDE_ROUTE_List != []:
                    ...
                    if text_line not in Printed_Lines:
                        ...
                        Config_Change.append(text_line)
                        Printed_Lines.append(text_line)
                    text_line = (' - Dest_Object is "%s", interface in is "%s", routing is:' %(this_Dst_Obj[0], t_If_Name))
                    if text_line not in Printed_Lines:
                        print (text_line)
                        Config_Change.append(text_line)
                        Printed_Lines.append(text_line)
                        for n in WIDE_ROUTE_List:
                            print ('   %s\t\t via %s' %(n[0],n[1]))
                            Config_Change.append('   %s' %n)
                        print('!')
                        Config_Change.append('!')
...
##                BEST_ROUTE = ''
##                WIDE_ROUTE_List = []
##                for row_index_1, row_1 in ROUTE_IP_DF.iterrows():
##                    this_route = row_1['IPv4_Network']
##                    try:
##                        if this_Dst_Obj_IP.subnet_of(this_route):
##                            if BEST_ROUTE == '':
...
##                if t_this_Dst_Obj != '0.0.0.0/0':
##                    if BEST_ROUTE == '': #no best route found
##                        for this_route in ROUTE_IP_DF['IPv4_Network'].to_list():
##                            try:
##                                if this_route.subnet_of(this_Dst_Obj_IP):
##                                    Out_Interface = ROUTE_IP_DF.loc[ROUTE_IP_DF['IPv4_Network'] == this_route].Interface.to_list()[0]
##                                    WIDE_ROUTE_List.append([str(this_route), Out_Interface])
##                                    ACL_OUT_IF_COUNTER_dic[(t_If_Name,Out_Interface)] += 1
##                            except:
##                                print('Error at line 2719:')
##                                print('this_route = %s' %this_route)
##                                print('this_Dst_Obj_IP = %s' %this_Dst_Obj_IP)
##                                exit(1234)
##                    else:
##                        Out_Interface = ROUTE_IP_DF.loc[ROUTE_IP_DF['IPv4_Network'] == BEST_ROUTE].Interface.to_list()[0]
##                        ACL_OUT_IF_COUNTER_dic[(t_If_Name,Out_Interface)] += 1
...
##                        if this_Dst_Obj[0] not in list(Founded_Routes.keys()):
##                            Founded_Routes[this_Dst_Obj[0]] = Out_Interface
##                        # then check if object wider than routing
##                        ROUTE_IP_DF_bis = ROUTE_IP_DF.copy()
##                        Best_Route_Index = ROUTE_IP_DF.index[ROUTE_IP_DF['IPv4_Network'] == BEST_ROUTE].to_list()[0]
##                        ROUTE_IP_DF_bis = ROUTE_IP_DF_bis.drop(Best_Route_Index)
##                        for this_route in ROUTE_IP_DF_bis['IPv4_Network'].to_list():
...
##def ACL_Dest_Vs_Routing_Table(t_device, Config_Change, log_folder):
##
##    from Network_Calc import Sub_Mask_2,Sub_Mask_1,IPv4_to_DecList,Is_Dec_Overlapping
##    from tabulate import tabulate
##    import shelve
##    import ipaddress
##    import pandas as pd
...
##
##    ACL_OUT_IF_COUNTER_dic = {}
##    for t_IN_ifName1 in Nameif_List:
##        for t_IN_ifName2 in Nameif_List:
##            ACL_OUT_IF_COUNTER_dic[(t_IN_ifName1,t_IN_ifName2)] = 0
##
##    ROUTE_IP_DF = ROUTE_DF.copy()
##    ROUTE_IP_DF['IPv4_Network'] = ''
##    for row_index, row in ROUTE_IP_DF.iterrows():
##        try:
##            ROUTE_IP_DF.loc[row_index].IPv4_Network = ipaddress.IPv4Network(row.Network)
##        except:
##            try:
##                t_routename = row.Network.split('/')[0]
##                t_routeip = Name_dic[t_routename] +'/'+ row.Network.split('/')[1]
##                ROUTE_IP_DF.loc[row_index].IPv4_Network = ipaddress.IPv4Network(t_routeip)
##            except:
##                Config_Change.append('ERROR 1105 while converting %s to ipaddress\n' %row.Network)
##                print('ERROR 1105 while converting %s to ipaddress\n' %row.Network)
...
##                    Out_Interface = ROUTE_IP_DF.loc[ROUTE_IP_DF['IPv4_Network'] == BEST_ROUTE].Interface.to_list()[0]
##                    ACL_OUT_IF_COUNTER_dic[(t_If_Name,Out_Interface)] += 1
####                    if t_If_Name == 'EXTRANET':
####                        if Out_Interface == 'MPLS-BUONASERA':
####                            print('MPLS-BUONASERA:' + t_key)
####                            print('ACL_OUT_IF_COUNTER_dic(MPLS-BUONASERA) = %s' %ACL_OUT_IF_COUNTER_dic[(t_If_Name,Out_Interface)])
####                        if Out_Interface == 'BORSA-ITALIANA':
####                            print('BORSA-ITALIANA:' + t_key)
####                            print('ACL_OUT_IF_COUNTER_dic (BORSA-ITALIANA) = %s' %ACL_OUT_IF_COUNTER_dic[(t_If_Name,Out_Interface)])
##                    if this_Dst_Obj[0] not in list(Founded_Routes.keys()):
##                        Founded_Routes[this_Dst_Obj[0]] = Out_Interface
##
##                if WIDE_ROUTE_List != []:
...
##    ACL_OUT_IF_COUNTER_df = pd.DataFrame(ACL_OUT_IF_COUNTER_list, columns = ['IF_in' , 'IF_Out', 'Count'])
##    Config_Change.append(tabulate(ACL_OUT_IF_COUNTER_df,ACL_OUT_IF_COUNTER_df,tablefmt='psql',showindex=False))
##    #print(tabulate(ACL_OUT_IF_COUNTER_df,ACL_OUT_IF_COUNTER_df,tablefmt='psql',showindex=False))
##    return Config_Change
##
...
def Active_Capture(t_device, Config_Change, log_folder):
    ...
    try:
        engine = db.create_engine("postgresql://postgres:igiull0_123@localhost/ASA_Check")
        connection = engine.connect()
        Active_Capture = db.Table('Active_Capture', db.MetaData(), autoload=True, autoload_with=engine)
        My_Devices = db.Table('My_Devices', db.MetaData(), autoload=True, autoload_with=engine)
    except:
        print('=================[ Warning ]==================')
        print('DB not connected, some feature is unavailable\n')
    ...
    text = ('Active Capture older than %s days @ %s' %(Max_Capture_Age,hostname))
    utils_v2.Text_in_Frame (text, Config_Change, Print_also=1)

    if DB_Available:
        query = db.select([Active_Capture]).where(Active_Capture.columns.HostName=="%s" %hostname)
        ResultProxy = connection.execute(query)
        ResultSet = ResultProxy.fetchall()
    ...
    for n in range(0,len(Capture_Line)):
        if 'capture ' in Capture_Line[n]:
            ...
            temp_line = [hostname, t_capture_name, today, t_capture_content]
    ...
    if len(Capture_db) > 0:
        ...
        Capture_db = Capture_db.drop('ID', 1)
        #check if capture is new
        for row in Capture_df.itertuples():
            t_name = row.Name
            query = db.select([Active_Capture]).where(db.and_(Active_Capture.columns.HostName==hostname, Active_Capture.columns.Name==t_name))
            ...
            if len(t_Capture_db) == 0: #capture is new
                ...
                t_m = ''.join(t_Capture_db[4][0])
                t_m = re13.sub('', t_m) # remove counters
                if t_n == t_m:
                    t_today = datetime.date(int(today.split('-')[0]),int(today.split('-')[1]),int(today.split('-')[2]))
                    if (t_today-t_Capture_db[3][0]).days >= Max_Capture_Age:
                        Clear_Capture.append('no capture %s' %row.Name)
                    Capture_df.iloc[row.Index].First_Seen = (t_today-t_Capture_db[3][0]).days
                else:
                    #capture with same name but modified
                    query = db.delete(Active_Capture).where(db.and_(Active_Capture.columns.HostName==row.HostName, Active_Capture.columns.Name==row.Name))
                    results = connection.execute(query)
    ...
    Fix_FName   = log_folder + '/' + hostname___ + '-Capture-Fix.html'
    Merge_FName = log_folder + '/' + hostname___ + '-Capture-Merge.txt'
    Watch_Flist = []
    Merge_Flist = []
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
    Merge_Flist.append('!')
...
#    import ipaddress
    ...
    for find_this in Undeclared_NetObj_Used_List:
        if len(find_this.split()) == 1:
            ...
            print ('unexpected object length')
            exit(3)
        ...
        for n in range(1,len(show_run)):
            if show_run[n].startswith(line_to_find):
                ...
                if len(OBJ_GRP_NET_Dic[show_run[m].rstrip().split()[2]]) > 1:
                    Config_Change.append(show_run[m].rstrip())
                    try:
                        Config_Change.append(' network-object object %s' %Obejct_by_value_Dict[find_this][0])
                        Config_Change.append(' no%s' %line_to_find)
                        Config_Change.append('!')
                    except:
                        Config_Change.append('!%s is not a "network-object object"... ' %(find_this))
                        Config_Change.append('!')
                else:
                    #print('convert this to " object network ..."')
                    Config_Change.append('! '+ show_run[m].rstrip())
                    Config_Change.append('!convert this to " object network ..."')
                    Config_Change.append('!')
...
##    NUM_Most_Triggered   = 10  #numero di regole a maggiore hit che vengono visualizzate
...
##    Config_Change.append(tabulate(Expanded_ACL_df,Expanded_ACL_df,tablefmt='psql',showindex=False))
    ...
    for row_index, row in ACL_Lines_DF.iterrows(): #sto facendo i controlli solo sulle ACL applicate ad interfacce
        if row.Name not in list(Accessgroup_Dic_by_if.values()):
            ACL_Lines_DF = ACL_Lines_DF.drop(row_index)

    query = db.select([ACL_GROSS]).where(ACL_GROSS.columns.HostName=="%s" %hostname___)
    ResultProxy = connection.execute(query)
    ResultSet = ResultProxy.fetchall()
    ACL_GROSS_db=pd.DataFrame(ResultSet)
    today = datetime.datetime.now().strftime('%Y-%m-%d')
    ...
    if len(ACL_GROSS_db) == 0: # New Device
        ...
        for row_index, row in ACL_Lines_DF.iterrows():
            N_of_ACL_NEW += 1
            query = db.insert(ACL_GROSS).values(HostName    =hostname___,
                                                First_Seen  =today,
                                                Name        =row.Name,
                                                Line        =row.Line,
                                                Type        =row.Type,
        ...
        ACL_GROSS_db = ACL_GROSS_db.drop('ID', 1)
        t_today = datetime.date(int(today.split('-')[0]),int(today.split('-')[1]),int(today.split('-')[2]))
        ...
        for row_index, row in ACL_Lines_DF.iterrows():
            ...
            if len(t_ACL_GROSS_db) == 0: # ACL LINE is new
                N_of_ACL_NEW += 1
                #print(' ACL LINE is new... writing to DB')
                query = db.insert(ACL_GROSS).values(HostName    =hostname___,
                                                    First_Seen  =today,
                                                    Name        =row.Name,
                                                    Line        =row.Line,
                ...
                if int(row.Hitcnt) > int(t_ACL_GROSS_db.Hitcnt):
                    ...
                    query = db.update(ACL_GROSS).where(db.and_(ACL_GROSS.c.HostName==hostname___, ACL_GROSS.c.Hash==t_hash)).values(Updated_Vals)
                    results = connection.execute(query)

                elif int(row.Hitcnt) == int(t_ACL_GROSS_db.Hitcnt):
                    t_Days = (t_today-t_ACL_GROSS_db.First_Seen.item()).days
                    t_line = 'access-list %s %s %s %s %s %s %s %s %s inactive' %(row.Name, row.Type, row.Action, row.Service, row.Source, row.S_Port, row.Dest, row.D_Port, row.Rest)
                    t_line = re_space.sub(' ',t_line)

                    if 'inactive' in row.Inactive:
                        # check if to be deleted
                        if (t_today-t_ACL_GROSS_db.First_Seen.item()).days >= Max_ACL_Inactive_Age:
                            ...
                            temp_no_inactive.append(['%s' %(t_Days), '%s' %(t_line)])
                            Fix_FList_Inactive.append('no %s' %(t_line))
                        ...
                        if row.Action.lower() == 'deny':
                            continue
                        elif (t_today-t_ACL_GROSS_db.First_Seen.item()).days >= Max_ACL_HitCnt0_Age:
                            ...
                            Fix_FList_DeltaHit0.append('%s' %(t_line))
                    ...
                    N_of_ACL_Resetted += 1
                    #print('ACL con Hitcount resettati, aggiorno DB') if (DEBUG_LEVEL == 0) else ''
                    #print('ACL con Hitcount resettati, aggiorno DB')
        ...
        Fix_FName   = log_folder + '/' + hostname___ + '-Inactive_ACL-Fix.html'
        Merge_Flist = []
        Merge_FName = log_folder + '/' + hostname___ + '-Inactive_ACL-Merge.txt'
        Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
        Merge_Flist.append('!')
        ...
        try:
            with open("%s/%s"%(html_folder,Watch_FName),mode="w") as html_file:
                ...
                html_file.write('       </tr></thead>\n')
                html_file.write('       <tbody>\n')
                for row_index, row in temp_no_inactive_DF.iterrows():
                    html_file.write('       <tr>\n')
                    for t_col_index in range(0,N_Cols):
                        ...
                        if t_col_index == N_Cols-1:
                            new_line = ''
                            for t_word in temp_no_inactive_DF.iloc[row_index][t_col_index].split():
                                if t_word in Blu_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Blu_Color, t_word)
                                elif t_word in Red_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Red_Color, t_word)
                            ...
                            html_file.write('           <td>%s</td>\n' %temp_no_inactive_DF.iloc[row_index][t_col_index])
                    html_file.write('       </tr>\n')
                html_file.write('       </tbody>\n')
                html_file.write('   </table>\n')
                html_file.write('</div>\n')
        ...
        try:
            with open("%s/%s"%(html_folder,Watch_FName_2),mode="w") as html_file:
                ...
                html_file.write('       </tr></thead>\n')
                html_file.write('       <tbody>\n')
                for row_index, row in temp_inactive_below_DF.iterrows():
                    html_file.write('       <tr>\n')
                    for t_col_index in range(0,N_Cols):
                        #print('row_index = %s' %row_index)
                        #print('t_col_index = %s' %t_col_index)
                        if t_col_index == N_Cols-1:
                            new_line = ''
                            for t_word in temp_inactive_below_DF.iloc[row_index][t_col_index].split():
                                if t_word in Blu_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Blu_Color, t_word)
                                elif t_word in Red_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Red_Color, t_word)
                            ...
                            html_file.write('           <td>%s</td>\n' %temp_inactive_below_DF.iloc[row_index][t_col_index])
                    html_file.write('       </tr>\n')
                html_file.write('       </tbody>\n')
                html_file.write('   </table>\n')
                html_file.write('</div>\n')
        ...
        Fix_FName   = log_folder + '/' + hostname___ + '-Deltahitcnt0_ACL-Fix.html'
        Merge_Flist = []
        Merge_FName = log_folder + '/' + hostname___ + '-Deltahitcnt0_ACL-Merge.txt'
        Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
        Merge_Flist.append('!')
        ...
        try:
            with open("%s/%s"%(html_folder,Watch_FName),mode="w") as html_file:
                ...
                html_file.write('       </tr></thead>\n')
                html_file.write('       <tbody>\n')
                for row_index, row in temp_yo_inactive_DF.iterrows():
                    html_file.write('       <tr>\n')
                    for t_col_index in range(0,N_Cols):
                        #print('row_index = %s' %row_index)
                        #print('t_col_index = %s' %t_col_index)
                        if t_col_index == N_Cols-1:
                            new_line = ''
                            for t_word in temp_yo_inactive_DF.iloc[row_index][t_col_index].split():
                                if t_word in Blu_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Blu_Color, t_word)
                                elif t_word in Red_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Red_Color, t_word)
                            ...
                            html_file.write('           <td>%s</td>\n' %temp_yo_inactive_DF.iloc[row_index][t_col_index])
                    html_file.write('       </tr>\n')
                html_file.write('       </tbody>\n')
                html_file.write('   </table>\n')
                html_file.write('</div>\n')
        ...
        try:
            with open("%s/%s"%(html_folder,Watch_FName_2),mode="w") as html_file:
                ...
                html_file.write('       </tr></thead>\n')
                html_file.write('       <tbody>\n')
                for row_index, row in temp_yo_inactive_below_DF.iterrows():
                    html_file.write('       <tr>\n')
                    for t_col_index in range(0,N_Cols):
                        #print('row_index = %s' %row_index)
                        #print('t_col_index = %s' %t_col_index)
                        if t_col_index == N_Cols-1:
                            new_line = ''
                            for t_word in temp_yo_inactive_below_DF.iloc[row_index][t_col_index].split():
                                if t_word in Blu_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Blu_Color, t_word)
                                elif t_word in Red_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Red_Color, t_word)
                            ...
                            html_file.write('           <td>%s</td>\n' %temp_yo_inactive_below_DF.iloc[row_index][t_col_index])
                    html_file.write('       </tr>\n')
                html_file.write('       </tbody>\n')
                html_file.write('   </table>\n')
                html_file.write('</div>\n')
        ...
        for row_index, row in ACL_GROSS_db.iterrows():
            t_hash = row.Hash
            Bool_check = ('Hash == "%s"' %(t_hash))
            ...
            if len(t_ACL_Lines_DF) == 0: # ACL LINE is no longer in config
                ...
                if Header_Printed == False:
                    ...
                    #print('\n!--- ACL removed from DB ---')
                    Header_Printed = True
                query = db.delete(ACL_GROSS).where(db.and_(ACL_GROSS.c.HostName==hostname___, ACL_GROSS.c.Hash==row.Hash))
                ResultProxy = connection.execute(query)
                t_line = ['@',row.HostName,'access-list',row.Name,row.Line,row.Type,row.Action,row.Service,row.Source,row.S_Port,row.Dest,row.D_Port,row.Rest,row.Inactive,row.Hitcnt,row.Hash]
...
##        pass
    ...
    print('Total number of updated ACL is %s' %N_of_ACL_Incremented)
    Config_Change.append('Total number of updated ACL is %s' %N_of_ACL_Incremented)
...
# ===============================================
    Temp_Config_Change = []
    re_space = re.compile(r'  +') # two or more spaces
    text = ('First %s Triggered ACL' %NUM_Most_Triggered)
    utils_v2.Text_in_Frame (text, Config_Change, Print_also=1)
    #print('line1')
...
##    with shelve.open(tf_name) as shelve_obj: Accessgroup_Dic_by_if = shelve_obj['0']
    ...
    for t_ACL in ACL_Names:
        ...
        for row_index, row in temp_df.iterrows():
            ...
            if LOOP_INDEX > (ITEMS/STEPS)*BINS:
                print ('...%s%%' %int(BINS*100/STEPS)); BINS = BINS + 1
            temp_df.loc[row_index,'#'] = int(row.Line.split()[1])
        ttt = temp_df.sort_values('#')
        temp_df = ttt.reset_index()
        temp_df.drop(['index'], 1)
...
##            if LOOP_INDEX > (ITEMS/STEPS)*BINS:
##                print ('...%s%%' %int(BINS*100/STEPS)); BINS = BINS + 1
            if t_Processed_ACLs == NUM_Most_Triggered:
                break
...
##                continue
                ...
                if row.Delta_HitCnt > 0:
                    ...
                    percent = round(row_index/(temp_df_NRows-1)*100,2) if temp_df_NRows-1 else 0
                    t0_line = 'Diff_HitCnt = %s' %(row.Delta_HitCnt)
                    t1_line = '%s%%' %(percent)
                    t2_line = 'access-list %s %s %s %s %s %s %s %s %s %s %s (hitcnt=%s) %s' %(row.Name, row.Line, row.Type, row.Action, row.Service, row.Source, row.S_Port, row.Dest, row.D_Port, row.Rest, row.Inactive, row.Hitcnt, row.Hash)
                    ...
                    if row.Action == 'deny':
                        ...
                        [Move_to_Line, out_fnc] = Check_Dec_Shadowing(t_device, t2_line, log_folder)
                        Temp_Config_Change.append('\n---'  + t2_line)
                        for n in out_fnc:
                            Temp_Config_Change.append(n)
                        if Move_to_Line != -1:
                            if int(row.Line.split()[1]) == int(max(Incremental_Line,(Move_to_Line))):
...
##                        else:
##                            Move_to_Line = (Check_Shadowing(t_device, t2_line, log_folder))[0]
##                            Temp_Config_Change.append('---'  + t2_line)
##                            for n in (Check_Shadowing(t_device, t2_line, log_folder))[1]:
##                                Temp_Config_Change.append(n)
##                            Temp_Config_Change.append('can be moved up to line %s' %(Move_to_Line+1))
...
def DB_For_OBJNET(t_device, Config_Change, log_folder):
    ...
    try:
        ...
        Config_Change.append('Can not connect to DB...')
        return Config_Change
    today = datetime.datetime.now().strftime('%Y-%m-%d')
    ...
    #read values from DB
    OBJNET_db = db.Table('OBJNET', db.MetaData(), autoload=True, autoload_with=engine)
    query = db.select([OBJNET_db])
    ResultProxy = connection.execute(query)
    ResultSet = ResultProxy.fetchall()
    OBJNET_db_df = pd.DataFrame(ResultSet)
    if len(OBJNET_db_df) == 0:
        # New DB
        for t_key in Obejct_by_value_Dict_one.keys():
            query = db.insert(OBJNET_db).values(Obj_Value = t_key,
    ...
    OBJNET_db_df.columns = ['Obj_Value','Obj_Name','Last_Seen']
...
##Last_Seen   = date
    ...
    try:
        ...
        print('ERROR! cannot save excel file %s.... is it open?' %Excel_IP_Report_FileName)
        Config_Change.append('ERROR! cannot save excel file.... is it open?')
    writer.close()
...
##        exit(0)
    try:
        ...
        print('file %s/%s___Show_nat.txt not found! @ CREATE VARIABLES' %(log_folder,hostname___))
        exit(0)
...
def Check_Dec_Shadowing(t_device, ACL_Line, log_folder):
    #from tabulate import tabulate
    #from Network_Calc import Sub_Mask_2
    from Network_Calc import Proto_Map
    #from Network_Calc import Tot_Shadow_List
...
##        return Config_Change
    ...
    if len(ACL_Line_Expanded_DF_Print) > 200:
    ...
    for index_1 in range(len(ACL_Line_Expanded_DF)-1,-1,-1):
        ...
        if (item1_Source == [[0,0]] and item1_Destin == [[0,0]]):
            #Temp_Config_Change.append('Line can not be moved...')
            Last_Hitted_Line.append(int(row1.Line.split()[1]))
            continue
...
def Check_NAT(t_device, Config_Change, log_folder):
    ...
    try:
        engine = db.create_engine("postgresql://postgres:igiull0_123@localhost/ASA_Check")
        connection = engine.connect()
        Show_NAT_DB = db.Table('Show_NAT_DB', db.MetaData(), autoload=True, autoload_with=engine)
        My_Devices = db.Table('My_Devices', db.MetaData(), autoload=True, autoload_with=engine)
    except:
        print('=================[ Warning ]==================')
        print('DB not connected, some feature is unavailable\n')
    ...
    ResultProxy = connection.execute(query)
    ResultSet = ResultProxy.fetchall()
    Show_NAT_DB_df = pd.DataFrame(ResultSet)
    today = datetime.datetime.now().strftime('%Y-%m-%d')
    t_today = datetime.date(int(today.split('-')[0]),int(today.split('-')[1]),int(today.split('-')[2]))
        ...
        for row_index, row in Show_NAT_DB_df.iterrows():
            ...
            if len(t_Show_NAT_DF) == 0: # NAT LINE is no longer in config
                ...
                if Header_Printed == False:
                    ...
                    #print('\n!--- NAT removed from DB ---')
                    Header_Printed = True
                query = db.delete(Show_NAT_DB).where(db.and_(Show_NAT_DB.c.HostName==hostname___, Show_NAT_DB.c.Nat_Line==row.Nat_Line))
                ResultProxy = connection.execute(query)
    ...
    if len(Show_NAT_DB_df) == 0:   # new device
        ...
        for row_index, row in Show_NAT_DF.iterrows():
            ...
            query = db.insert(Show_NAT_DB).values(  HostName     = hostname___,
                                                    Last_Seen    = today,
                                                    Section      = row.Section,
                                                    Line_N       = row.Line_N,
                                                    IF_IN        = row.IF_IN,
...
##        N_of_NAT_New = 0
                                    ...
                                    if (row_0['IF_OUT'] == row_2['IF_OUT']) or (row_2['IF_OUT']=='any'):
                                        ...
                                        for row_0_SRC in row_0['SRC_Origin']:
                                            try:
                                                ...
                                                print('Cannot convert "%s" to IPv4' %row_0_SRC)
                                                continue
                                                #print(row_0_SRC)
                                            #row_0_SRC_IP = ipaddress.IPv4Network(row_0_SRC, strict=False)
                                            for row_2_SRC in row_2['SRC_Origin']:
                                                try:
                                                    ...
                                                    print('Cannot convert "%s" to IPv4' %row_2_SRC)
                                                    continue
                                                    #print(row_2_SRC)
                                                #row_2_SRC_IP = ipaddress.IPv4Network(row_2_SRC, strict=False)
                                                ...
                                                if row_0_SRC_IP.subnet_of(row_2_SRC_IP) or row_0_SRC_IP.supernet_of(row_2_SRC_IP):
                                                    ...
                                                    for row_0_DST in row_0['DST_Origin']:
                                                        try:
                                                            ...
                                                            print('Cannot convert "%s" to IPv4' %row_0_DST)
                                                            continue
                                                            #print(row_0_DST)
                                                        #row_0_DST_IP = ipaddress.IPv4Network(row_0_DST, strict=False)
                                                        for row_2_DST in row_2['DST_Origin']:
                                                            try:
                                                                ...
                                                                print('Cannot convert "%s" to IPv4' %row_2_DST)
                                                                continue
                                                                #print(row_2_DST)
                                                            #row_2_DST_IP = ipaddress.IPv4Network(row_2_DST, strict=False)
                                                            else:
    ...
    for t_field in Moved_NAT:
        ...
        if t_field[1] == 1:
            ...
            t_new_item.append('show nat | i %s' %' '.join(temp[5:]))
    ...
    txt_FName   = log_folder + '/' + hostname___ + '-Most_Triggered_NAT.txt'
    try:
        with open(txt_FName,mode="w") as my_file:
            #my_file.writelines(line for line in Most_Triggered_NAT_txt)
    ...
    Watch_FName = log_folder + '/' + hostname___ + '-Most_Triggered_NAT-Watch.html'
    try:
        with open(Watch_FName,mode="w") as html_file:
            #html_file.writelines(line for line in Watch_FList)
            html_file.writelines(Watch_FList)
    ...
    Fix_FName   = log_folder + '/' + hostname___ + '-Most_Triggered_NAT-Fix.html'
    try:
...
##            text = ('Most %s Tr_Hit Triggered NAT' %N_NAT_Most_Triggered)
##            utils_v2.Text_in_Frame (text, Config_Change, Print_also=1)
##            Show_NAT_DB_df = Show_NAT_DB_df.sort_values('Delta_Tr_Hit',ascending=False)
##            #Config_Change.append('\n --- NAT has %s rows---' %(NRows_Show_NAT_DB_df))
##            t_Processed_NATs = 0
##            Incremental_Line = 1
##            Temp_Tr_Nat = []
##            for row_index, row in Show_NAT_DB_df.iterrows():
...
##                        elif row['Section'] == 3:
##                            NAT_Position = row['Line_N'] + Section_1_NAT_Lines + Section_2_NAT_Lines
##                        percent = round(NAT_Position/(NRows_Show_NAT_DB_df-1)*100,2) if NRows_Show_NAT_DB_df-1 else 0
##                        Temp_Tr_Nat.append(['%s' %(row.Delta_Tr_Hit), '%s%%' %(percent), '[%s|%s]'%(row.Section, row.Line_N), '%s' %row.Nat_Line])
##                        t_Processed_NATs += 1
##
##            Temp_Tr_Nat_DF = pd.DataFrame(Temp_Tr_Nat, columns = ['Diff_Tr' , '%', 'Section', 'NAT'])
##            Config_Change.append(tabulate(Temp_Tr_Nat_DF,Temp_Tr_Nat_DF,tablefmt='psql',showindex=False))
##            temp = 0
##            for row_index, row in Temp_Tr_Nat_DF.iterrows():
##                temp = temp + float(row['%'].strip('%'))
##            temp = temp / N_NAT_Most_Triggered
##            Config_Change.append('\n Average NAT percent position = %.1f\n' %temp)
##
##
##            text = ('Most %s Un_Hit Triggered NAT' %N_NAT_Most_Triggered)
##            utils_v2.Text_in_Frame (text, Config_Change, Print_also=1)
##            Show_NAT_DB_df = Show_NAT_DB_df.sort_values('Delta_Un_Hit',ascending=False)
##            t_Processed_NATs = 0
##            Incremental_Line = 1
##            Temp_Un_Nat = []
##            for row_index, row in Show_NAT_DB_df.iterrows():
...
##                            NAT_Position = row['Line_N'] + Section_1_NAT_Lines + Section_2_NAT_Lines
##                        percent = round(NAT_Position/(NRows_Show_NAT_DB_df-1)*100,2) if NRows_Show_NAT_DB_df-1 else 0
##                        Temp_Un_Nat.append(['%s' %(row.Delta_Un_Hit), '%s%%' %(percent), '[%s|%s]'%(row.Section, row.Line_N), '%s' %row.Nat_Line])
##                        t_Processed_NATs += 1
##
##            Temp_Un_Nat_DF = pd.DataFrame(Temp_Un_Nat, columns = ['Diff_Un' , '%', 'Section', 'NAT'])
##            Config_Change.append(tabulate(Temp_Un_Nat_DF,Temp_Un_Nat_DF,tablefmt='psql',showindex=False))
##            temp = 0
##            for row_index, row in Temp_Un_Nat_DF.iterrows():
##                temp = temp + float(row['%'].strip('%'))
##            temp = temp / N_NAT_Most_Triggered
##            Config_Change.append('\n Average NAT percent position = %.1f\n' %temp)
    ...
    for row_index, row in ROUTE_IP_DF.iterrows():
        try:
            ROUTE_IP_DF.loc[row_index].Network = ipaddress.IPv4Network(row.Network)
        except:
    ...
    Merge_Flist = []
    Merge_FName = log_folder + '/' + hostname___ + '-Inactive_NAT-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
    Merge_Flist.append('!')
    ...
    Watch_FName   = log_folder + '/' + hostname___ + '-Inactive_NAT-Watch.html'
    try:
        with open(Watch_FName,mode="w") as html_file:
            #html_file.writelines(line for line in Watch_FList)
            html_file.writelines(Watch_FList)
    ...
    Watch_FName_2 = log_folder + '/' + hostname___ + '-Inactive_NAT-Watch_2.html'
    try:
    ...
    Fix_FName   = log_folder + '/' + hostname___ + '-Inactive_NAT-Fix.html'
    try:
        with open(Fix_FName,mode="w") as html_file:
            #html_file.writelines(line for line in Fix_FList)
            html_file.writelines(Fix_FList)
    ...
    Watch_FName   = log_folder + '/' + hostname___ + '-Deltahitcnt0_NAT-Watch.html'
    try:
        with open(Watch_FName,mode="w") as html_file:
            #html_file.writelines(line for line in Watch_FList)
            html_file.writelines(Watch_FList)
    ...
    Watch_FName_2 = log_folder + '/' + hostname___ + '-Deltahitcnt0_NAT-Watch_2.html'
    try:
        with open(Watch_FName_2,mode="w") as html_file:
            #html_file.writelines(line for line in Watch_FList)
            html_file.writelines(Watch_FList)
    ...
    Fix_FName   = log_folder + '/' + hostname___ + '-Deltahitcnt0_NAT-Fix.html'
    try:
        with open(Fix_FName,mode="w") as html_file:
            #html_file.writelines(line for line in Fix_FList)
            html_file.writelines(Fix_FList)
    ...
    Think_FName   = log_folder + '/' + hostname___ + '-Most_Triggered_NAT-Think.html'
...
##object network DNS_G8
## range 172.16.210.1 172.16.210.3
    N_Range_IP_Obj = 0
    N_Max_Range_IP = 0
...
##                print (text_line)
                ...
                Out = []
                t_Out = Where_Used(t_device, t_obj_key, log_folder, Out)
                if t_Out:
                    for line in t_Out:
