...
from utils_v2 import Write_Think_File, File_Save_Try
...
def Get_ASA_Commands(Device,log_folder,Status_Flag):
...
##        device_connection.send_command('enable')
    ...
    for t_Command in Commands:
        ...
        while retries <5:
            try:
                output.append("%s\n\n%s\n\n" %(t_Command, device_connection.send_command(t_Command,max_loops=50000,delay_factor=1,read_timeout=300*retries)))
    ...
    try:
        ...
        raise OSError("Can't write to destination file (%s/%s.txt)!" % (FW_log_folder,hostname___))
...
def Split_Show_run(Device, Show_Line, log_folder):
    ...
    try:
        with open("%s/%s.txt"%(FW_log_folder,hostname___),"r") as f:
            l = f.readlines()
    except:
...
##        print("Can't open file %s/%s___%s.txt for writing" %(FW_log_folder,hostname___,Show_Line.title().strip().replace(' ','_')))
    ...
    tf_name = ("%s/%s___%s.txt"%(FW_log_folder,hostname___,Show_Line.title().strip().replace(' ','_')))
...
##    confdiff_html      =  '../../app/templates/confdiff.html'
    ...
    try:
        with open("%s/%s___Show_Running-Config.txt" %(FW_log_folder,hostname___),"r", errors='replace') as f:
...
def ACL_VS_Interface(t_device, Config_Change, log_folder):
    ...
    try:
        with open("%s/%s___Show_Nameif.txt"%(FW_log_folder,hostname___),"r") as f:
            l = f.readlines()
    except:
...
##        Config_Change.append('no object-group network %s' %n)
    ...
    Merge_Flist = []
    Merge_FName = FW_log_folder + '/' + hostname___ + '-ObjNet_Not_Applied-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S\n\n'))
    ...
    Merge_Flist = []
    Merge_FName = FW_log_folder + '/' + hostname___ + '-ObjGrpNet_Not_Applied-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S\n\n'))
    ...
    Merge_Flist = []
    Merge_FName = FW_log_folder + '/' + hostname___ + '-ObjSvc_Not_Applied-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S\n\n'))
    ...
    Merge_Flist = []
    Merge_FName = FW_log_folder + '/' + hostname___ + '-ObjGrpSvc_Not_Applied-Merge.txt'
    Merge_Flist.append(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S\n\n'))
            ...
            if len(t_ACL_Lines_DF_check) == 0:
                ...
                if DB_Available:
                    ...
                    if len(t_Delta_HitCnt) > 1:
                        print('too many rows with the same hash = %s' %t_key)
                        t_Delta_HitCnt = t_Delta_HitCnt.drop_duplicates()
...
##            print('!')
        ...
        ResultProxy = connection.execute(query)
        ResultSet = ResultProxy.fetchall()
        ACL_Summary_db = pd.DataFrame(ResultSet)

        t_Prct_ACL_Space_TCP = round(sum(ACL_Summary_db[10])/len(ACL_Summary_db[10]),1) if not (len(ACL_Summary_db[10])==0) else 0
        t_Prct_ACL_Space_UDP = round(sum(ACL_Summary_db[11])/len(ACL_Summary_db[11]),1) if not (len(ACL_Summary_db[11])==0) else 0
        t_Prct_ACL_Space_ICMP = round(sum(ACL_Summary_db[9])/len(ACL_Summary_db[9]),1) if not (len(ACL_Summary_db[9])==0) else 0
...
#  (=================================================)
    try:
        with open("%s/Sankey_ACL_Chart.js"%(Err_folder),"r") as f:
            l = f.readlines()
    except:
        print('ERROR!!! file %s/Sankey_ACL_Chart.js not found!' %(Err_folder))
...
def Active_Capture(t_device, Config_Change, log_folder):
    ...
    DB_Available = True
    try:
        engine = db.create_engine("postgresql://postgres:%s@localhost/ASA_Check" %PostgreSQL_PW)
        connection = engine.connect()
    ...
    if DB_Available:
        ...
        query = db.select(Active_Capture).where(Active_Capture.c.HostName==hostname)
    ...
    Clear_Capture = []
    if len(Capture_db) > 0:
        Capture_db.columns = ['ID','Name','First_Seen','Content','HostName']
        ...
        if len(Capture_df) > 0:
            ...
            query = db.delete(Active_Capture).where(Active_Capture.columns.HostName==hostname)
            results = connection.execute(query)
    else:
        #capture is new
...
##    Config_Change.append(tabulate(Expanded_ACL_df,Expanded_ACL_df,tablefmt='psql',showindex=False))
    ...
    if len(ACL_GROSS_db) == 0: # New Device
        ...
        for row in ACL_Lines_DF.itertuples():
            ...
            t_ACL_GROSS_db = (ACL_GROSS_db.query(Bool_check)).drop_duplicates()
                ...
                if int(row.Hitcnt) > int(t_ACL_GROSS_db.Hitcnt):
                    ...
                    if 'inactive' in row.Inactive:
                        ...
                        if row.Action.lower() == 'deny':
                            ...
                            temp_yo_inactive.append([t_Days, '%s (hitcnt=%s)' %(t_line.replace(' inactive',''), t_ACL_GROSS_db.Hitcnt.item())])
                            Fix_FList_DeltaHit0.append(t_line)
            ...
            with open("%s/%s"%(html_folder,Watch_FName),mode="w") as html_file:
                ...
                for row in temp_no_inactive_DF.itertuples():
                    ...
                    for t_col_index in range(0,N_Cols):
                        ...
                        if t_col_index == N_Cols-1:
                            new_line = ''
                            for t_word in temp_no_inactive_DF.iloc[row.Index][t_col_index].split():
                                if t_word in Blu_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Blu_Color, t_word)
                                elif t_word in Red_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Red_Color, t_word)
                            ...
                            html_file.write('           <td>%s</td>\n' %temp_no_inactive_DF.iloc[row.Index][t_col_index])
                    html_file.write('       </tr>\n')
                html_file.write('       </tbody>\n')
                html_file.write('   </table>\n')
                html_file.write('</div>\n')
            ...
            with open("%s/%s"%(html_folder,Watch_FName_2),mode="w") as html_file:
                ...
                for row in temp_inactive_below_DF.itertuples():
                    ...
                    for t_col_index in range(0,N_Cols):
                        ...
                        if t_col_index == N_Cols-1:
                            new_line = ''
                            for t_word in temp_inactive_below_DF.iloc[row.Index][t_col_index].split():
                                if t_word in Blu_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Blu_Color, t_word)
                                elif t_word in Red_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Red_Color, t_word)
                            ...
                            html_file.write('           <td>%s</td>\n' %temp_inactive_below_DF.iloc[row.Index][t_col_index])
                    html_file.write('       </tr>\n')
                html_file.write('       </tbody>\n')
                html_file.write('   </table>\n')
                html_file.write('</div>\n')
            ...
            with open("%s/%s"%(html_folder,Watch_FName),mode="w") as html_file:
                ...
                for row in temp_yo_inactive_DF.itertuples():
                    ...
                    for t_col_index in range(0,N_Cols):
                        ...
                        if t_col_index == N_Cols-1:
                            new_line = ''
                            for t_word in temp_yo_inactive_DF.iloc[row.Index][t_col_index].split():
                                if t_word in Blu_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Blu_Color, t_word)
                                elif t_word in Red_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Red_Color, t_word)
                            ...
                            html_file.write('           <td>%s</td>\n' %temp_yo_inactive_DF.iloc[row.Index][t_col_index])
                    html_file.write('       </tr>\n')
                html_file.write('       </tbody>\n')
                html_file.write('   </table>\n')
                html_file.write('</div>\n')
            ...
            with open("%s/%s"%(html_folder,Watch_FName_2),mode="w") as html_file:
                ...
                for row in temp_yo_inactive_below_DF.itertuples():
                    ...
                    for t_col_index in range(0,N_Cols):
                        ...
                        if t_col_index == N_Cols-1:
                            new_line = ''
                            for t_word in temp_yo_inactive_below_DF.iloc[row.Index][t_col_index].split():
                                if t_word in Blu_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Blu_Color, t_word)
                                elif t_word in Red_Words:
                                    new_line = new_line + '<font color="%s"> %s </font>' %(Red_Color, t_word)
                            ...
                            html_file.write('           <td>%s</td>\n' %temp_yo_inactive_below_DF.iloc[row.Index][t_col_index])
                    html_file.write('       </tr>\n')
                html_file.write('       </tbody>\n')
                html_file.write('   </table>\n')
                html_file.write('</div>\n')
...
##    with shelve.open(tf_name) as shelve_obj: Accessgroup_Dic_by_if = shelve_obj['0']
    ...
    for t_ACL in ACL_Names:
        ...
        for row in temp_df.itertuples():
            ...
            if LOOP_INDEX > (ITEMS/STEPS)*BINS:
                print ('...%s%%' %int(BINS*100/STEPS)); BINS = BINS + 1
            temp_df.loc[row.Index,'#'] = int(row.Line.split()[1])
...
##        exit(0)
    try:
        with open("%s/%s___Show_Nat_Detail.txt"%(FW_log_folder,hostname___),"r") as f:
